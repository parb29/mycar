...
<button type="button" class="btn btn-primary btn-xs" data-toggle="modal" data-target="#<%=articleList.get(i).getNNUM()%>" title="삭제"><span class="glyphicon glyphicon-remove"></span>삭제</button>
  ...
  <ul class="pagination pagination-sm">
		...
		<%}else{ %>
	<li class="page-item">
      <a class="page-link" href="noticeAdList.no?page=<%=startPage-1 %>" title="<%=startPage-1 %> 페이지로 이동">이전
      </a>
    </li>
		<%} %>
		...
		<%}else{ %>
	<li class="page-item">
		<a class="page-link" href="noticeAdList.no?page=<%=endPage+1 %>" title="<%=endPage %> 페이지로 이동">다음
		</a>
		    </li>
  </ul>
</nav>
