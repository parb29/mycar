...
<%
	...
	String fieldName = pageInfo.getFieldName();
	String viewId = pageInfo.getviewId();
...
<div class="container">
	<h2><a href="resAdSearchAction.re" title="예약 목록">예약 목록</a></h2>
   ...
   <button class='btn btn-default pull-right btn-dark' title="예약추가" id="insert" onclick="location.href='resAdInsertAction.re'">예약추가</button>
     <form action="resAdSearchAction.re" name="searchform" method="post">
	    ...
	    <div class="input-group" id="serch">
	      <input type="search" class="form-control" name="viewId" id="viewId" value="${pageInfo.viewId}" placeholder="검색어를 입력하세요.">
...
if (resList != null && listCount > 0) {
	...
	<tr>
		...
		%>
			<tr>
				...
				<button type="button" class="btn btn-default btn-xs" data-toggle="modal" data-target="#<%=resList.get(i).getRnum()%>" title="예약 취소"><span class="glyphicon glyphicon-remove"></span> 예약취소</button>
		           ...
		           <div class="modal-body">
		               ...
		               <div class="modal-footer">
		                <button type="button" onclick="location.href='resAdDeleteAction.re?rnum=<%=resList.get(i).getRnum()%>&page=${pageInfo.page}&id=${pageInfo.viewId}&fieldName=${pageInfo.fieldName}'" class="btn btn-secondary btn" title="예약취소">예</button>
		                 <button type="button" class="btn btn-secondary btn" data-dismiss="modal" title="아니오">아니오</button>
	...
	<section id="pageList">
		...
		<% if (nowPage <= 5) { %>
		<% } else { %>
		<li><a href="resAdSearchAction.re?page=<%=startPage-1 %>&fieldName=${pageInfo.fieldName}&viewId=${pageInfo.viewId}" title="이전 페이지로 이동" >이전</a></li>
		<%}  %>
		...
		<% } else { %>
		<li><a href="resAdSearchAction.re?page=<%=a %>&fieldName=${pageInfo.fieldName}&viewId=${pageInfo.viewId}" title="<%=a %>페이지로 이동"><%=a %></a></li>
		<% } %>
		<% } %>
		...
		<%} else { %>
		<li><a href="resAdSearchAction.re?page=<%=endPage+1 %>&fieldName=${pageInfo.fieldName}&viewId=${pageInfo.viewId}" title="다음 페이지로 이동">다음</a></li>
		<%} %>
		</ul>
	</section>
